...
function renderTech(){
  ...
  const jumpBtn = document.getElementById('jumpTotal');
  if(jumpBtn){
    jumpBtn.onclick = () => document.getElementById('totalSummary')?.scrollIntoView({behavior:'smooth', block:'center'});
  }
...
function renderSummary(){
  ...
  document.getElementById('summaryLines').innerHTML=lines.join('') || '<p class="small" style="text-align:center">Select parts and quantities</p>'; document.getElementById('total').textContent=money(total); const jump=document.getElementById('jumpTotal'); if(jump) jump.textContent = 'Total ' + money(total) + ' ↓';
}
