...
function logout(){ localStorage.removeItem('lgdsRole'); localStorage.removeItem('lgdsAdminPassword'); state={ role:null,data:null,quantities:{},search:'',pendingSearch:'',adminPassword:'',openCats:{} }; render(); }
...
function renderTech(){
  ...
  document.getElementById('app').innerHTML = `<div class="wrap"><div class="topbar"><div><h1>Parts Calculator</h1><p class="small">Technician mode</p></div><div class="tools techTools"><div class="searchrow"><input class="input search" id="search" placeholder="Search part, category, price..." value="${escapeAttr(searchValue)}"/><button class="btn primary searchBtn" id="searchBtn">Search</button><button class="btn" id="clearSearch">Clear search</button></div><button class="btn" id="clearQty">Clear quantities</button><button class="btn danger" id="logout">Logout</button></div></div>${state.search ? `<div class="searchStatus">Showing ${resultCount} result${resultCount===1?'':'s'} for: <b>${escapeHtml(state.search)}</b></div>` : ''}<div id="cats"></div><div class="summary"><div id="summaryLines"></div><div class="total" id="total">$0.00</div></div></div>`;
